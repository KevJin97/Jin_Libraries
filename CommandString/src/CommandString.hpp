...
public:
	...
	std::vector<std::size_t> locate(std::vector<std::string> find);	//locate a set of strings in parsed
...
std::vector<std::size_t> CommandString::locate(std::vector<std::string> find)
{
	std::vector<std::size_t> locations;
	...
	if (this->parsed.size())
	{
		for (std::size_t index = 0; index < this->parsed.size(); ++index)
		{
			for (std::size_t listindex = 0; listindex < find.size(); ++listindex)
			if (this->parsed[index].compare(find[listindex]) == 0)
			{
				locations.push_back(index);
			}
		}
